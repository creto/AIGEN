...
(SQL Server /         │               lista para compilar y ejecutar
 PostgreSQL)          │
                      ▼
              0 errores · 0 warnings
              2769 archivos generados (S16)
```
...
└── Templates/
    ...
    └── Solution/                 ← Plantillas de solución (generadas 1 vez)
        ├── program.scriban         ← Program.cs completo: DI + JWT + SignalR + Health
        ├── appsettings.scriban
        ├── dbcontext.scriban
        ├── solution.scriban
        ├── api_csproj.scriban
        ...
        ├── signalr_hub.scriban     ← NotificationHub + INotificationService  ← NUEVO S16
        ├── app_component.scriban   ← Angular AppComponent con menú + toast SignalR
        ├── angular_menu_service.scriban
        ├── angular_signalr_service.scriban ← SignalRService con reconexión  ← NUEVO S16
...
aigen generate --config aigen.json --no-interactive
       │
       ▼
1. Deserializar GeneratorConfig (JSON → C#, PropertyNameCaseInsensitive=true)
       │
       ▼
...
4. FileGeneratorService
   ...
   └── GenerateSolutionAsync() — 1 vez
       ├── dbcontext.scriban
       ├── program.scriban (DI + Swagger + JWT + SignalR + Audit + Cache + Health)
       ├── auth_controller.scriban
       ├── menu_controller.scriban
       ├── signalr_hub.scriban → API/Hubs/NotificationHub.cs     ← NUEVO S16
       ├── menu_seed.scriban → sql/menu_seed.sql
       ├── app_component.scriban (menú dinámico + toast SignalR)
       ├── angular_menu_service.scriban
       ├── angular_signalr_service.scriban → core/services/signalr.service.ts  ← NUEVO S16
...
audit_table_filters[] → [{entity_name, fields, fields_quoted}]
                         generado estáticamente desde auditing.tables
...
all_tables_script[]   → [{repository_name, service_name, class_name, controller_name,
                           ...
                           entity_name, entity_name_plural, has_estado, ...]}
                         Incluye snake_case aliases explícitos
```
...
│   │   ├── Repositories/{Entidad}Repository.cs ← EF/Dapper/SP según crudStrategy
│   │   ├── Configurations/{Entidad}Configuration.cs ← Fluent API (solo EF)
│   │   └── Interceptors/AuditSaveChangesInterceptor.cs
│   │       ├── ICurrentUserContext
│   │       ├── SystemUserContext        ← fallback
│   │       └── HttpCurrentUserContext   ← lee claims JWT reales
│   └── Cache/CacheService.cs           ← ICacheService + Memory/Redis
│
└── {ProjectName}.API
    ├── Controllers/
    ...
    ├── Hubs/
    │   └── NotificationHub.cs          ← SignalR Hub + INotificationService ← S16
    │       ├── NotificationHub : Hub
    │       ├── OnConnectedAsync() → grupos user_{name} + role_{rol}
    │       ├── SendToUser(), SendToRole(), Broadcast()
    │       ├── INotificationService    ← interfaz inyectable
    │       └── SignalRNotificationService ← usa IHubContext<NotificationHub>
    └── Program.cs
        ├── AddSignalR() + MapHub<NotificationHub>("/hubs/notifications")  ← S16
        ├── AddAuthentication().AddJwtBearer()
        ├── GET /health (liveness) + GET /ready (readiness)
        └── DI completo: repositorios, servicios, caché, auditoría
```

### 5.2 Angular — Estructura de servicios core

```
frontend/src/app/
├── app.component.ts          ← Menú dinámico signal + PanelMenu + Toast SignalR
├── app.config.ts             ← withInterceptors([authInterceptor, errorInterceptor])
├── app.routes.ts             ← Rutas generadas por entidad
└── core/
    └── services/
        ├── auth.service.ts   ← JWT signals + login/logout/me
        ├── menu.service.ts   ← GET /api/menu + getPrivilegios()
        └── signalr.service.ts← HubConnection + reconexión + notification$ ← S16
```

### 5.3 SignalR — Flujo de notificaciones

```
Backend (NotificationHub)              Angular (SignalRService)
        │                                       │
...
### 5.4 Repositorio — Árbol de decisión
...
### 5.5 Soft-delete — Árbol de decisión
...
|---|---|---|
| `PA_TablaAdd` | INSERT + retorna registro | `SCOPE_IDENTITY()` — thread-safe |
| `PA_TablaUpdate` | UPDATE parámetros opcionales | `ISNULL(@param, col)` + `and Estado=1` |
| `PA_TablaDelete` | Soft-delete + `ValidarDependencias` | `CREATE OR ALTER` — idempotente |
...
| `PA_TablaGetByFilter` | Búsqueda dinámica paginada | `sp_executesql` + `OFFSET/FETCH` |
| `PA_TablaToggleEstado` | `CASE WHEN Estado=1 THEN 0 ELSE 1` | Nuevo — no existía en histórico |

---

## 8. SignalR — Arquitectura de notificaciones

### Grupos de conexión
```
user_{username}   → notificaciones personales al usuario
role_{roleName}   → notificaciones a todos los usuarios de un rol
(sin grupo)       → Broadcast a todos los conectados
```

### INotificationService — Uso desde controllers
```csharp
// Inyectar en cualquier controller o servicio
public class DocumentosController : ControllerBase
{
    ...
    {
        // ... lógica de creación ...
        await _notifications.NotifyRoleAsync("Admin",
            "success", "Nuevo documento creado", new { id = doc.Id });
        return CreatedAtAction(...);
    }
}
```

### Angular SignalRService — Uso en componentes
```typescript
...
## 9. Modelo de menú — Esquema Seguridad
...
```
1. Login → AuthService guarda JWT
2. AppComponent.ngOnInit() → MenuService.getMenu()
3. GET /api/menu (Bearer token)
4. MenuController extrae roles[] del claim JWT
5. JOIN TP_Menu → TR_MenuRol WHERE AspNetRoles IN (roles)
6. BuildTree() — árbol Padre/Orden recursivo
7. Angular mapToMenuItems() → MenuItem[] PrimeNG
8. p-panelmenu renderiza árbol dinámico
9. SignalRService.startAsync() — conecta hub  ← S16
10. notification$.subscribe() → toast PrimeNG  ← S16
...
## 10. Auditoría — Arquitectura de capas
...
## 11. Health endpoints — Cloud-native
...
## 12. Configuración completa — `aigen.json`
...
Paso 7: CODE_SUMMARY.md (conteo por capa)
Paso 8: dotnet run API (opcional -RunApi)
```

### Perfiles disponibles

| Perfil | Comando | Archivos | Estado |
|---|---|---|---|
| SqlServer monolito | `-Db SqlServer` | 2769 · 0 errores | ✅ |
...
| S15 | Menús dinámicos + 11 privilegios SGDEA | ✅ |
| S16 | SignalR ✅ + MassTransit ⏳ + 2FA ⏳ | ⏳ 50% |
...
| S23 | Documentación + `dotnet tool install -g aigen` | ⏳ |
