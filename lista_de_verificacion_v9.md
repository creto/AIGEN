# 🗺️ AIGEN — Derrotero de Desarrollo v9
**Proyecto:** AI-Powered Software Generator (AIGEN)  
**Stack:** .NET Core 8 · Scriban · EF Core · Dapper · Angular 18 · PrimeNG · SignalR  
...
**Última actualización:** Abril 2026 — Semana 16 en progreso (SignalR completado)
...
Semana 16 ████████████░░░░░░░░  50% ⏳  SignalR ✅ + MassTransit ⏳ + 2FA ⏳
...
- [x] `aigen.ps1` v2 — launcher interactivo con menú BD × modo
- [x] `rebuild_and_generate.ps1` v4 — perfiles SqlServer/Postgres/SP/Microservices
...
- [x] `audit_interceptor.scriban`:
  ...
  - `HttpCurrentUserContext` — lee claims JWT reales (`preferred_username` → `name` → `Identity.Name`) — **resuelto en S15**
...
- [x] **`sp_crud.scriban`** — 8 SPs por tabla con estándar TiGlobal modernizado:
...
## ⏳ SEMANA 16 — Notificaciones real-time + Mensajería + 2FA
> **Estado: EN PROGRESO ⏳** — SignalR completado (50%)

### SignalR — COMPLETADO ✅
- [x] **`signalr_hub.scriban`** — `NotificationHub : Hub` generado en `{ns}.API/Hubs/`:
  - `OnConnectedAsync()` — agrega al grupo `user_{username}` y `role_{rol}` según claims JWT
  - `OnDisconnectedAsync()` — limpia grupos
  - `SendToUser()`, `SendToRole()`, `Broadcast()` — métodos del hub
- [x] **`INotificationService` + `SignalRNotificationService`** — inyectable en cualquier controller/servicio
  - `NotifyUserAsync()`, `NotifyRoleAsync()`, `BroadcastAsync()`
  - Registrado como `AddSingleton<INotificationService, SignalRNotificationService>()`
- [x] **`program.scriban`** actualizado:
  - `builder.Services.AddSignalR()`
  - `app.MapHub<NotificationHub>("/hubs/notifications")`
- [x] **`angular_signalr_service.scriban`** — `SignalRService` Angular:
  - `HubConnectionBuilder` con `accessTokenFactory` (usa JWT del `AuthService`)
  - Reconexión automática: `[0, 2s, 5s, 10s, 30s]`
  - `notification$: Subject<Notification>` — observable de notificaciones
  - `connected$: BehaviorSubject<boolean>` — estado de conexión
  - `startAsync()` / `stopAsync()` — ciclo de vida
  - `notifyUser()`, `notifyRole()` — métodos cliente → servidor
- [x] **`app_component.scriban`** actualizado:
  - `ToastModule` PrimeNG conectado a `notification$`
  - `getSeverity()` — mapea `type` → severity PrimeNG (success/error/warn/info)
  - `startAsync()` al login, `stopAsync()` al logout
- [x] **2769 archivos · 0 errores · 0 warnings ✅**

### MassTransit — PENDIENTE ⏳
- [ ] `message_consumer.scriban` — consumer base generado por proyecto
- [ ] `program.scriban` — `AddMassTransit()` condicional según config
- [ ] `appsettings.scriban` — sección `"MessageBroker"` (RabbitMQ / Azure Service Bus)
- [ ] `FeaturesConfig.MessageBroker` — `None` | `RabbitMQ` | `AzureServiceBus`
- [ ] `infrastructure_csproj.scriban` — paquetes MassTransit condicionales

### 2FA — PENDIENTE ⏳
- [ ] `SecurityConfig.TwoFactor` — `None` | `TOTP` | `SMS` | `Email`
- [ ] `auth_controller.scriban` — endpoints 2FA: `POST /api/auth/2fa/setup` · `POST /api/auth/2fa/verify`
- [ ] Angular: `angular_2fa_component.scriban` — QR code + verificación TOTP
...
| Prefijo | TableType | HasFullCrud | SPs generados |
|---|---|---|---|
| `TM_` | Movement | ✅ | ✅ 8 SPs completos |
| `TB_` | Basic | ✅ | ✅ |
| `TBR_` | BasicRelated | ✅ | ✅ |
| `TP_` | Parameter | ✅ | ✅ |
...
### Modo Monolito (`crudStrategy: direct`) — 2769 archivos
```
Generated/
├── Doc4Us.sln
├── sql/
...
│   └── Doc4Us.API/
│       ├── Controllers/           ← CRUD + AuthController + MenuController
│       ├── Hubs/
│       │   └── NotificationHub.cs ← SignalR Hub + INotificationService
│       └── Program.cs             ← DI + Swagger + JWT + SignalR + Health + Audit
└── frontend/
    └── src/app/
...
## 🐛 Registro de bugs resueltos (acumulado S1-S16)
...
| 16 | `Banner.cs` | Nombres microservicios como colores Spectre | Alto | ✅ S12 |
| 17 | `entity.scriban` | Navigation properties hacia entidades externas | Alto | ✅ S12 |
| 18 | `ScribanTemplateEngine` | `microservice_class_names_array` sin ScriptObject nativo | Alto | ✅ S12 |
| 19 | `ForeignKeyMetadata` | Tipos FK/PK incompatibles (int→string) | Crítico | ✅ S14 |
| 20 | `ForeignKeysScript` | `ScriptObject.Import` no renombra propiedades anidadas | Alto | ✅ S14 |
| 21 | `FileGeneratorService` | `NavigationPropertyName` sin sync tras desambiguación | Alto | ✅ S14 |
| 22 | `TemplateContext` | `JwtKey` no expuesto — key vacía en appsettings | Alto | ✅ S14 |
...
| 26 | `SqlServerSchemaReader` | `RawIdx`/`RawFK`/`RawKey` records corrompidos | Crítico | ✅ S14 |
| 27 | `AuditSaveChangesInterceptor` | No filtra por campos configurados | Medio | ✅ S15 |
| 28 | `ICurrentUserContext` | `SystemUserContext` retorna "system" siempre | Alto | ✅ S15 |
| 29 | `sp_crud.scriban` | `sp_ToggleEstado` faltante | Medio | ✅ S15 |
| 30 | `FileGeneratorService` | `mixed` strategy definida pero no implementada | Alto | ✅ S15 |
| 31 | `repository_sp.scriban` | `pk_ts_type` retornaba tipo TypeScript no C# | Alto | ✅ S15 |
| 32 | `repository_sp.scriban` | `GetPagedAsync` retornaba tuple en lugar de PagedResponse | Alto | ✅ S15 |
| 33 | `dbcontext.scriban` | `using Configurations` siempre incluido | Medio | ✅ S15 |
| 34 | `program.scriban` | Sin health endpoints `/health` y `/ready` | Medio | ✅ S15 |
| 35 | `GenerateCommand` | `--no-interactive` solo generaba tablas CRUD completo | Alto | ✅ S15 |
| 36 | `AllTablesScript` | Snake_case aliases faltantes para plantillas de solución | Alto | ✅ S15 |
| 37 | `aigen.ps1` | `OrderedDictionary.ContainsKey()` no existe — usar `.Contains()` | Alto | ✅ S16 |
| 38 | `aigen.ps1` | Flags pasados como array en lugar de splatting correcto | Alto | ✅ S16 |
| 39 | `rebuild_and_generate.ps1` | Em dash `—` corrupto como `â€"` — encoding issue | Medio | ✅ S16 |
...
+-------------------------------------------------------------+
|  NOTIFICACIONES (S16) — IMPLEMENTADO                        |
|  SignalR Hub · grupos por usuario/rol · Toast PrimeNG      |
+-------------------------------------------------------------+
|  AUTORIZACIÓN (S15) — IMPLEMENTADO                          |
|  Menús por rol · 11 privilegios SGDEA · AuthGuard          |
+-------------------------------------------------------------+
|  AUTENTICACIÓN (S11) — IMPLEMENTADO                         |
|  JWT · Refresh HttpOnly · BCrypt · OWASP A01-A09           |
+-------------------------------------------------------------+
|  AUDITORÍA (S13+S15) — IMPLEMENTADO                         |
...
## 🔑 Decisiones Arquitectónicas Tomadas (acumulado S1-S16)
...
| Caché por tabla en `aigen.json` | Tablas TP_* ideales para cache largo |
| SignalR para notificaciones real-time | Nativo .NET, escalable con Redis backplane |
| Grupos SignalR por `user_{name}` y `role_{rol}` | Notificaciones personalizadas sin broadcasts masivos |
| `INotificationService` inyectable | Desacopla controllers del hub SignalR directamente |
| Reconexión automática SignalR `[0,2s,5s,10s,30s]` | Back-off progresivo — no satura el servidor |
| MassTransit para mensajería | Abstracción sobre RabbitMQ/Azure SB — cambiar provider sin código |
| `crudStrategy` configurable | `direct` · `storedProcedures` · `mixed` |
| `mixed` strategy por tabla con `SpTables[]` | Granularidad tabla a tabla — migración gradual SP → EF |
| Schema `[API]` para SPs | Separación en BD — usuarios solo tienen EXECUTE |
| `sp_executesql` en GetByFilter | Sargability de índices |
| `SCOPE_IDENTITY()` en lugar de `@@identity` | Thread-safe en concurrencia |
| Templates en archivos vs tabla `API.paramGen` | Versionables en Git, debuggeables |
| `--no-interactive` flag en CLI | Permite uso en CI/CD sin intervención manual |
| `AllTablesScript` con snake_case aliases | `ScriptObject` Scriban no renombra keys internas |
| `p-panelmenu` PrimeNG para menú jerárquico | Árbol multinivel nativo + lazy loading |
| `GET /health` + `GET /ready` en todos los modos | Liveness vs readiness — patrón cloud-native |
| Health endpoints `AllowAnonymous` | Health checks no requieren autenticación |
| `OrderedDictionary.Contains()` no `.ContainsKey()` | PowerShell: `OrderedDictionary` no tiene `ContainsKey` |
| Splatting `-Db $dbValue @extraFlags` en PowerShell | Pasar parámetros nombrados correctamente al script |
| Docker multistage build | Imagen final mínima |
